...
if ($id > 0) {
    ...
    $approvedValue = $statusMode === 'active_inactive' ? 'active' : 'approved';
    ...
    $conn->query("UPDATE testimonials SET `$statusCol`='$approvedValue' WHERE id=" . $id);

    // Keep both columns in sync if they both exist
    if (isset($cols['status']) && isset($cols['statut']) && $statusCol === 'status') {
        $conn->query("UPDATE testimonials SET `statut`='$approvedValue' WHERE id=" . $id);
    }
    if (isset($cols['status']) && isset($cols['statut']) && $statusCol === 'statut') {
        $conn->query("UPDATE testimonials SET `status`='$approvedValue' WHERE id=" . $id);
    }
}
